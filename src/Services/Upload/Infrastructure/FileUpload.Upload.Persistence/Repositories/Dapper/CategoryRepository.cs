...
namespace FileUpload.Upload.Persistence.Repositories.Dapper
{
    public class CategoryRepository : DapperRepository<Category>, ICategoryRepository
    {
        ...
        {
            var query = $"select * from category where user_id = {_sharedIdentityService.GetUserId}";
        ...
        {
            var query = $"select * from category where id = {id} and user_id = {_sharedIdentityService.GetUserId}";
        ...
        {
            var query = $"insert into category (title, created_date, user_id) values(@Title, @CreatedDate, @ApplicationUserId) returning id";
        ...
        {
            var query = $"delete from category where id = {id} and user_id = {_sharedIdentityService.GetUserId}";
            return await _connection.ExecuteAsync(query) > 0;
        }

        public override async Task<bool> Update(Category entity)
        {
            var query = $"update category set title = @Title where id = @Id and user_id = @ApplicationUserId";
        ...
        {
            var query = $"update category set title = {title} where id = {id} and user_id = {_sharedIdentityService.GetUserId}";
